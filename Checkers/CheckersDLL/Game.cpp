...
{
	int BlackScore = 12 - std::count_if(BoardPosition->cbegin(), BoardPosition->cend(),
		[](std::pair<Position, Checker*> SomeObj) {
			return SomeObj.second->GetType() == Checker::Type::WHITE;
		});
	int WhiteScore = 12 - std::count_if(BoardPosition->cbegin(), BoardPosition->cend(),
		[](std::pair<Position, Checker*> SomeObj) {
	...
	for (int Y = rect.top; Y < rect.bottom; ++Y) {
		double Colour = 255 - Y * 255. / rect.bottom;
		HPEN hPen = CreatePen(BS_SOLID, 1, RGB(255, Colour, Colour));
		SelectObject(hdc, hPen);
		MoveToEx(hdc, rect.bottom + 10, Y, NULL);
		LineTo(hdc, rect.right, Y);
		DeleteObject(hPen);
	}

	std::wstring WS;
	if (*ActivePlayer == Checker::Type::WHITE)
		WS = L"BLUE";
	else
		WS = L"RED";

	HFONT hFont = CreateFont(100, 40, 1, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, VARIABLE_PITCH, L"Harlow Solid Italic");
	HFONT hFont2;
	HFONT hFont3 = CreateFont(45, 25, 1, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, VARIABLE_PITCH, L"Algerian");

	SetBkMode(hdc, TRANSPARENT);

	SelectObject(hdc, hFont);
	TextOut(hdc, 525, 10, L"Checker", 8);

	if (WhiteScore < 12 && BlackScore < 12) {

		hFont2 = CreateFont(55, 30, 1, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
			OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, VARIABLE_PITCH, L"Bauhaus 93");

		SelectObject(hdc, hFont2);
		TextOut(hdc, 625, 100, WS.c_str(), WS.size());
		WS = L"B (" + std::to_wstring(WhiteScore) + L") : R (" + std::to_wstring(BlackScore) + L")";
		SelectObject(hdc, hFont3);
		TextOut(hdc, 575, 170, WS.c_str(), WS.size());

	}
	else {
		hFont2 = CreateFont(50, 20, 1, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
			OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, VARIABLE_PITCH, L"Bauhaus 93");

		SelectObject(hdc, hFont2);
		TextOut(hdc, 475, 225, L"Thank you for game!!!", 22);
		if (*ActivePlayer == Checker::Type::WHITE)
			WS = L"Winner is BLUE";
		else
			WS = L"Winner is RED";
		SelectObject(hdc, hFont3);
		TextOut(hdc, 525, 170, WS.c_str(), WS.size());
	}
